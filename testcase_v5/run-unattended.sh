...
#   the core carries the buffer pool that every prior customer core LACKED), the
...
#   TC_BIN           path to tc-idr binary        (default: ./bin/tc-idr-linux-amd64,
#                                                   falls back to ./build/tc-idr)
...
#   BACKSTOP_GRACE   kill tc-idr this many seconds past its own deadline (default: 300)
#
...
TC_BIN="${TC_BIN:-}"
if [[ -z "${TC_BIN}" ]]; then
  if [[ -x ./bin/tc-idr-linux-amd64 ]]; then TC_BIN=./bin/tc-idr-linux-amd64
  elif [[ -x ./build/tc-idr ]]; then TC_BIN=./build/tc-idr
  else echo "FATAL: no tc-idr binary found; set TC_BIN or run 'make build-linux'." >&2; exit 2; fi
fi
...
log "process core limit   = $(ulimit -c)  (this runner sets 'unlimited' for tc-idr's child)"
...
log "          core carries the buffer pool (the artifact prior customer cores lacked)."
...
# ── Launch tc-idr (its own max_runtime_seconds bounds the soak) ───────────────
( ulimit -c unlimited; exec "${TC_BIN}" --config "${TC_CONFIG}" "${TC_VERB}" ) \
  >> "${WORKDIR}/tc-idr.out" 2>&1 &
TC_PID=$!
log "tc-idr launched pid=${TC_PID}; output -> ${WORKDIR}/tc-idr.out"
...
# ── Supervise: poll for a crash; exit when tc-idr exits ──────────────────────
...
# If tc-idr is still alive here, the loop broke on a crash — stop it.
if kill -0 "${TC_PID}" 2>/dev/null; then
  log "stopping tc-idr (pid=${TC_PID}) after crash detection"
...
log "=== runner end: tc-idr exited, NO crash detected. ==="
