...
func TestAcceptMinimal(t *testing.T) {
	...
	if cfg.Database.Write.Schema != "byfile_tc" {
		t.Fatalf("schema: got %q want byfile_tc", cfg.Database.Write.Schema)
	}
}
...
func TestRejectGoldenFiles(t *testing.T) {
	cases := []struct {
		...
		{"reject-teams-zero.json", "init.teams"},
		{"reject-max-parallel.json", "max_parallel_teams"},
...
func TestRejectCases(t *testing.T) {
	cases := []struct {
		...
		{"TeamsZero", func(c *Config) { c.Init.Teams = 0 }, "init.teams"},
		{"MaxParallelTooMany", func(c *Config) {
			c.Init.Teams = 1
			c.Init.MaxParallelTeams = 5
		}, "max_parallel_teams"},
...
func TestValidateCollectsMultipleErrors(t *testing.T) {
	...
	for _, want := range []string{"batch_size", "init.teams", "encryption.mode"} {
