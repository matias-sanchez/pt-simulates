...
func validateNoise(cfg *Config) []error {
	...
	if n.InsertWorkers > 0 && n.InsertStartID > 0 {
		seededHi := cfg.Init.StartID + int64(cfg.Init.RowsPerTeam)*int64(cfg.Init.Teams)
	...
	if n.UpdateHotRowsPerTeam < 0 {
		errs = append(errs, fmt.Errorf("noise.update_hot_rows_per_team must be >= 0 (got %d)", n.UpdateHotRowsPerTeam))
	}
	if n.UpdateHotRowsPerTeam > 0 && cfg.Init.RowsPerTeam > 0 && n.UpdateHotRowsPerTeam > cfg.Init.RowsPerTeam {
		errs = append(errs, fmt.Errorf(
			"noise.update_hot_rows_per_team (%d) must be <= init.rows_per_team (%d)",
			n.UpdateHotRowsPerTeam, cfg.Init.RowsPerTeam))
	}
	if n.PoolMaxConns < 0 {
...
func validateInit(cfg *Config) []error {
	...
	i := cfg.Init
	if i.Teams < 1 {
		errs = append(errs, fmt.Errorf("init.teams must be >= 1 (got %d)", i.Teams))
	}
	if i.RowsPerTeam < 1 {
		errs = append(errs, fmt.Errorf("init.rows_per_team must be >= 1 (got %d)", i.RowsPerTeam))
	}
	if i.MaxParallelTeams < 1 {
		errs = append(errs, fmt.Errorf("init.max_parallel_teams must be >= 1 (got %d)", i.MaxParallelTeams))
	}
	if i.Teams >= 1 && i.MaxParallelTeams > i.Teams {
		errs = append(errs, fmt.Errorf(
			"init.max_parallel_teams (%d) must be <= init.teams (%d)",
			i.MaxParallelTeams, i.Teams))
	}
...
func validateRun(cfg *Config) []error {
	...
	if r.BatchSize != 10 {
		errs = append(errs, fmt.Errorf(
			"run.batch_size must be 10 (got %d) — matches production IDR cursor",
			r.BatchSize))
	}
	...
	if r.MaxParallelTeams < 1 {
		errs = append(errs, fmt.Errorf(
			"run.max_parallel_teams must be >= 1 (got %d)", r.MaxParallelTeams))
	}
	...
	if r.TeamLogIntervalBatches < 1 {
		errs = append(errs, fmt.Errorf(
			"run.team_log_interval_batches must be >= 1 (got %d)",
			r.TeamLogIntervalBatches))
	}
	return errs
}
