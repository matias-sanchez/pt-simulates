...
def plot_qps_tps(
    ...
    for r in runs:
        ...
        t_rel = t_vals_sec[:]
        t_q, q_b = _bucketize_series(t_rel, qps, bucket_seconds)
        t_t, t_b = _bucketize_series(t_rel, tps, bucket_seconds)
        t_common, q_b, t_b = _align_two_series(t_q, q_b, t_t, t_b)
        if not t_common:
            continue
        t_hr = [t / 3600.0 for t in t_common]
        window = max(1, int(round(60.0 / max(bucket_seconds, 1.0))))
        if window > 1 and len(q_b) > window:
            ...
            tps_s = t_b
            t_smooth = t_hr
        qps_s = _mask_isolated_zeros(qps_s)
        tps_s = _mask_isolated_zeros(tps_s)
        if t_smooth:
    ...
    for ax, m in zip(axes, metrics.keys()):
        ...
        for name, (t_hr, y_vals) in prepared[m].items():
            ax.plot(t_hr, y_vals, linewidth=2.0, alpha=0.95, color=color_map[name], linestyle=style_map[name], label=name)
        ax.set_ylim(0, yhi)
...
def write_qps_csv(
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for r in runs:
            ...
            # Bucketize
            t_q, q_b = _bucketize_series(t_rel, q_vals, bucket_seconds)
            t_t, t_b = _bucketize_series(t_rel, tps_vals, bucket_seconds)
            # Align
            t_common, q_b, t_b = _align_two_series(t_q, q_b, t_t, t_b)

            for t_sec, qv, tv in zip(t_common, q_b, t_b):
                w.writerow([r.name, "Queries", f"{t_sec:.3f}", qv])
                w.writerow([r.name, "Transactions", f"{t_sec:.3f}", tv])
